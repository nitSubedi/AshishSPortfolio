...
// forked from design-sync lib/source-kit.mjs — synth-entry `export * from`
// drops default-exported components (window.<Global>.<Name> ends up
// undefined for every file that uses `export default`); also re-export each
// file's default under its resolved component name.
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // App entry files (main.jsx/index.jsx at the src root) call
    // ReactDOM.createRoot(...).render(...) as a module-level side effect —
    // bundling that runs it as soon as the script loads, and it throws when
    // there's no #root element in the page (the bundle-export smoke check's
    // bare test page), poisoning the whole entry IIFE before window.<Global>
    // ever gets assigned. Exclude it: it's the app's bootstrap, not a
    // reusable component.
    const APP_ENTRY_RX = /\/(main|index)\.(jsx|tsx)$/;
    ...
    for (const p of comps) {
      ...
      // `export * from` never re-exports a file's `default` — recover the
      // declared name (e.g. `export default function Button()`) and
      // re-export it under that name so window.<Global>.<Name> exists.
      const sf = project.addSourceFileAtPathIfExists(p);
      ...
      if (real && /^[A-Z][A-Za-z0-9]*$/.test(real)) {
        lines.push(`export { default as ${real} } from ${JSON.stringify(p)};`);
      }
    }
